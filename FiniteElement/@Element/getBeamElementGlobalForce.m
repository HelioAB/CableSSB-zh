function [ANodes_matrix,BNodes_matrix,InternalForce_A_matrix,InternalForce_B_matrix] = getBeamElementGlobalForce(obj,type_elems,type_force)
    arguments
    ...
    for row = 1:sz(1)
        elems = obj(row,:);
        sorted_elems = elems.sortByCenterPoint(direction_sort);
        % 获取内力结果
        switch type_force
            case 'Fx'
        ...
        if strcmp(type_elems,'Tower') && strcmp(type_force,'My')
            ...
            ANodes_matrix(sz(1),length(ANodes)) = Node();
            BNodes_matrix(sz(1),length(BNodes)) = Node();
            InternalForce_A_matrix = zeros(sz(1),length(InternalForce_A));
            InternalForce_B_matrix = zeros(sz(1),length(InternalForce_B));
        end
        ANodes_matrix(row,:) = ANodes;
        BNodes_matrix(row,:) = BNodes;
        InternalForce_A_matrix(row,:) = InternalForce_A;
        InternalForce_B_matrix(row,:) = InternalForce_B;
    end
end