function solveHangerTopCoord(obj,hanger,Pz_Hanger)
...
%     options.Axes = ax;
%     options.Figure = fig;
    result_cell = solveHangerTopP_NewtonMethod(obj,hanger,cable,tol);
...
%             assignin("base","P_Y_k0",P_Y_k0)
    ...
    [f0_x,f0_y,f0_z,result_struct] = solveHangerTopP_Newton_f(obj,hanger,cable,x0,y0,z0,result_struct);
...
%             P_X_k1 = -ones(size(P_Z_k1)).*sign_cable_tension(:,1)';
    ...
    while (norm(eps_x)>tol || norm(eps_y)>tol || norm(eps_z)>tol) && iter_count<=IterAmount
        ...
        [f1_x,f1_y,f1_z,result_struct] = solveHangerTopP_Newton_f(obj,hanger,cable,x1,y1,z1,result_struct);
        result_struct.xi = x1;
        result_struct.yi = y1;
        result_struct.zi = z1;
