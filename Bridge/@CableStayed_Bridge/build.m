function build(obj)
    ...
    % 主跨加劲梁
    L = 7.5*ones(1,68); % 该段加劲梁的分段情况，只需要输入每一段的正确比例就可以
    CoordA_girder1 = [157.2,0,-4.2]; % 加劲梁左端点A的位置，"左"代表 X向数值更小的方向。
    CoordB_girder1 = [667.2,0,-4.2]; % 加劲梁右端点B的位置
    girder1 = obj.buildGirder(CoordA_girder1,CoordB_girder1,L,Sec_girder,Mat_girder,ET_girder); % 根据比例L，对CoordA和CoordB线性插值
    
    Point_StayedCable_girder1_1 = girder1.findPoint('Interval','X','ascend',[30,15+zeros(1,14)]).sort('X'); % 这些点是靠近PointA的斜拉索作用点
    Point_StayedCable_girder1_2 = girder1.findPoint('Interval','X','descend',[30,15+zeros(1,14)]).sort('X'); % 这些点是靠近PointB的斜拉索作用点
    
    % 边跨1加劲梁
    L = 7.5+zeros(1,22);
    CoordA_girder2 = [-7.8,0,-4.2];
    ...
    % 桥塔2
    Coord_MoveTo_tower2 = [667.2,0,-87];
    ...
    % 辅助墩2
    Coord_MoveTo_pier2 = [757.2,0,-57.4];
