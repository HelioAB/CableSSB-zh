classdef Pier < Structure
    properties
        Method_Creating
        RigidBeam
    end
    methods
        function obj = Pier(coord_bottom,coord_top,L,section,material)
            arguments
                ...
                section = Section('Pier')
                material = Material('Pier')
            end
            obj = obj@Structure(section,material);

            if abs(coord_top(3) - coord_bottom(3) - sum(L)) > eps
                warning('Pier的上下端点间的Z向距离不等于每个分段之和，请检查')
            end
            if nargin ~= 0 
                % 创建并记录Point和Line
