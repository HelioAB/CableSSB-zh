function data = getInfluenceLineFromAnsys(obj,OutputMethodObj_analyzeInfluenceLine,num_GirderNodes,num_MonitoredNodes,num_MonitoredElems_Link,num_MonitoredElems_Beam) % 获取影响线结果，前提是已经使用analyzeInfluenceLine()进行分析
    ...
    % 导入数据    
    data = struct;
    data.num_GirderNodes = num_GirderNodes;
    data.num_MonitoredNodes = num_MonitoredNodes;
    ...
    columns = [length(num_MonitoredNodes),length(num_MonitoredNodes),length(num_MonitoredNodes),...
               length(num_MonitoredElems_Link),...
               length(num_MonitoredElems_Beam),length(num_MonitoredElems_Beam),length(num_MonitoredElems_Beam),length(num_MonitoredElems_Beam)];
...
function output_str = getInfluenceStr(OutputMethodObj,num_GirderNodes,num_MonitoredNodes,num_MonitoredElems_Link,num_MonitoredElems_Beam)
    % 
    if ~isempty(num_MonitoredNodes)
        str_nodes_define = ['! 定义考察节点',newline,...
                            ...
                            OutputMethodObj.outputArray(num_MonitoredNodes,'num_monitored_nodes'),newline,...
    ...
    if ~isempty(num_MonitoredElems_Link)
        str_links_define = ['! 定义考察Link单元',newline,...
                            ...
                            OutputMethodObj.outputArray(num_MonitoredElems_Link,'num_monitored_elems_link'),newline,...
                            '*dim,Fx_link,array,count_nodes,count_result_elems_link',newline,newline];
        str_links_get = ['    ! Link单元获取轴力',newline,...
                            '    *do,i,1,count_result_elems_link',newline,...
                            '        *get,Fx_link(t,i),Elem,num_monitored_elems_link(i),Smisc,1',newline,...
                            '    *enddo',newline];
        str_links_out = ['! Fx_link',newline,...
                            '*cfopen,InfluenceLine_Fx_link,txt',newline,...
                            str_multi_out(num_MonitoredElems_Link,'Fx_link'),...
                            '*cfclos',newline];
    else
        str_links_define = [];
        str_links_get = [];
    ...
    if ~isempty(num_MonitoredElems_Beam)
        str_beams_define = ['! 定义考察Beam单元',newline,...
                            ...
                            OutputMethodObj.outputArray(num_MonitoredElems_Beam,'num_monitored_elems_beam'),newline,...
    ...
    output_str = [  '! 导出影响线',newline,...
                    '/post1',newline,...
                    sprintf('resume,%s,db',OutputMethodObj.JobName),newline,newline,...
                    '! 加劲梁节点',newline,...
                    sprintf('count_nodes = %d',length(num_GirderNodes)),newline,...
                    OutputMethodObj.outputArray(num_GirderNodes,'num_girder_nodes'),newline,...
                    str_nodes_define,...
                    str_links_define,...
                    str_beams_define,...
                    '! 获取',newline,...
