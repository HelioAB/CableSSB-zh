function obj = HybridBridge_SpanMain(span_main)
    ...
    % 主跨加劲梁
    num_seg = span_main/7.5;
    L = 7.5*ones(1,num_seg); % 该段加劲梁的分段情况，只需要输入每一段的正确比例就可以
    CoordA_girder1 = [157.2,0,-4.2]; % 加劲梁左端点A的位置，"左"代表 X向数值更小的方向。
    CoordB_girder1 = [CoordA_girder1(1)+sum(L),0,-4.2]; % 加劲梁右端点B的位置
    girder1 = obj.buildGirder(CoordA_girder1,CoordB_girder1,L,Sec_girder,Mat_girder,ET_girder); % 根据比例L，对CoordA和CoordB线性插值
    
    Point_Hanger_girder1 = girder1.findPoint('Interval','X','ascend',[187.5,15+zeros(1,21)]); % 'ascend'时按X正方向距离PointA的距离，寻找girder1的点。这些点是吊索作用点
